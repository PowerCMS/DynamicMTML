...
use strict;
use Exporter;
our $powercms_util_version = '2.05';
...
=head2 powercms_util_version( )

    PowerCMS::Util のバージョンを返します。

=head2 build_tmpl( $app, $tmpl, \%args [, \%params ] )
...
=item * \%args - ( optional )
    ...
    MTオブジェクトの各オブジェクト ( MT::Blog,MT::Category, MT::Entry, MT::Author ) および
    タイムスタンプ ( start,end ) を指定します。例:
...
=head2 save_asset( $app, $blog, $params, $cb )
...
=item * $cb - ( optional )
    ...
    アイテムの直前に cms_pre_save.asset, 保存直後に cms_post_save.asset 及び
    cms_upload_file( cms_upload_image )コールバックをコールします。

=back

=head2 upload( $app, $blog, $name, $dir, \%params )
...
=head2 convert_gif_png( $image );

    $imageで指定したGIF形式のファイルをPNG形式のファイルに、PNG形式のファイルをGIF形式のファイルに変換します。

=head2 association_link( $app, $author, $role, $blog )
...
=head2 create_entry( $app, $blog, \%args, \%params[, $cb] )
...
=item * \%args  - ( required )
    ...
    id( エントリーのID )を指定した場合、エントリーが存在すれば上書きします。
    複数のカテゴリを指定した場合最初のカテゴリがプライマリカテゴリに指定されます。
...
=item * $cb - ( optional )
    ...
    エントリーの保存前に cms_pre_save.entry( page ) エントリーの保存後に cms_pre_save.entry( page )
    コールバックをコールします。

=back

=head2 make_entry( $app, $blog, \%args, \%params[, $cb] )

    create_entryへのエイリアスです。

=head2 write2file ( $path, $data, $type, $blog )
...
=head2 read_from_file( $path, $type, $blog )
...
=head2 move_file( $from, $to, $blog )

    $from から $to へファイルを移動します。
    $blogを指定した場合相対パス ( %r や %s を含むパス ) の使用が可能です。

=head2 copy_item( $from, $to, $blog )

    $from から $to へファイル ( 又はフォルダ ) をコピーします。
    $blogを指定した場合相対パス ( %r や %s を含むパス ) の使用が可能です。

=head2 remove_item( $remove, $blog )

    $remove ( フルパス ) のファイル ( フォルダ ) を削除します。
    $blogを指定した場合相対パス ( %r や %s を含むパス ) の使用が可能です。

=head2 relative2path( $path, $blog )

    相対パス ( %r や %s を含むパス ) をフルパスに変換します。

=head2 path2relative( $path, $blog )

    フルパスを相対パス ( %r や %s を含むパス ) に変換します。

=head2 path2url( $path, $blog )

    フルパスからURLを生成します。

=head2 relative2url( $path, $blog )

    相対パス ( %r や %s を含むパス ) をURLに変換します。

=head2 url2path( $url, $blog )

    URLからサーバーのフルパスを生成します。

=head2 site_path( $blog )

    ブログのサイト・パスを常に末尾の / ( スラッシュ ) を省略して返します。
    アーカイブ・パスの指定がある場合はアーカイブ・パスを返します。

=head2 archive_path( $blog )
...
=head2 valid_phone_number( $num )

    $num が電話番号の書式として妥当な場合 1 を返します。

=head2 valid_postal_code( $code )

    $code が郵便番号の書式として妥当な場合 1 を返します。

=head2 month2int( $month )

    月を表す文字列( 例: January, Jan, jan ) から数字を返します。

=head2 send_multipart_mail( \%args )
...
=item * \%args - ( required )
    ...
    %args = (
        ...
        cc        => $cc,        # optional
        bcc       => $bcc,       # optional
        attaches  => \@attaches, # optional
        cb_params => \$cb_params # optional
    );

    $cb_paramsには send_mail 関数に渡すコールバック用のパラメタを指定します。
    パラメタの指定方法については send_mail 関数のドキュメントを参照。
...
=item * \%args - ( required )
    ...
    %args = (
        Body     => \@body_lines,        # required
        Attaches => \@attache_file_info, # optional
    );
    ...
    @attache_file_info = (
        {
            Path => '添付ファイルのパス', # 必須パラメータ、存在しなければ無視される
            Type => 'MIMEタイプ',       # MIME::Entity的に必須、デフォルトmime_type()で生成
            Encoding => 'エンコード方式' # なければMIME::Entityに推測させる
        },
    );

=back

=head2 send_mail( $from, $to, $subject, $body
                  [, \@cc, \@bcc, \%params_for_callback ] )

=head2 send_mail( \%args, \%params_for_callback )
...
=item * \%params_for_callbacks - ( optional )

    呼び出し側でコールバックされたときに渡すものを自由に指定します。
    以下のようにkeyを入れてください。keyがない場合は、'default'が自動的に入ります。例:
...
=head3 trigger callbacks

=over 4

=item * pre_send_mail( $app, \%args, \%spec )
...
=head2 get_mail( $server, $id, $passwd, $protocol, $delete )
...
=head2 make_zip_archive( $directory, $out, $files )
...
=item * $files     - ( required )
    ...
    ディレクトリ内の特定のファイル(複数可)を対象にアーカイブを作成する場合、$files にファイルパスの
    配列を渡します。

=back

=head2 current_user( $app )
...
=head2 current_blog( $app )

    実行中のアプリケーションの MT::Blog オブジェクトを返します。

=head2 get_content( $uri, $utf8, $file )
...
=head2 extract_content( $start, $end, $data, $cont )
...
=head2 regex_extract( $pattern, $data )

    正規表現パターンを指定してデータを抽出します。

=head2 str_replace( $from, $to, $data )

    文字列を置換します。

=head2 regex_replace( $pattern, $replace, $data )
...
=head2 ftp_new( $server, $account, $password, $param )

    Net::FTP オブジェクトを生成してFTPサーバーに接続します。

=head2 ftp_get( $ftp, $cwd, $rfile, $lfile, $mode )
...
=item * $mode  - ( optional; default 'binary' )

    転送モード ( 省略した場合 'binary' )

=back

=head2 ftp_put( $ftp, $cwd, $file, $mode, $params )
...
=item * pre_ftp_put( $app, \$ftp, \$cwd, \$file, \$mode, \$params )

    アップロードの直前に呼び出されます。コールバックが偽を返すとアップロードを中止します。

=item * post_ftp_put( $app, \$ftp, \$cwd, \$file, \$mode, \$params )

    アップロードの直後に呼び出されます。

=back

=head2 ftp_mkdir( $ftp, $cwd, $dir )
...
=head2 set_upload_filename( $file )

    URLに不適切な文字列を含むパス名( 日本語を含むパス名など ) をランダムな文字列のパスに変換します。

=head2 uniq_filename( $file )

    set_upload_filename を実行した後、同名のファイルが存在すれば連番を付けてユニークなファイルパス
    を生成します。

=head2 get_children_filenames( $directory, $pattern )

=head2 get_children_files( $directory, $pattern )
...
=head2 if_power_edit( $app )

=head2 is_power_edit( $app )

    実行中のアプリケーションの mode ( __mode ) がブログ記事 / ウェブページの一括編集の場合に
    1 を返します。

=head2 if_application( $app )

=head2 is_application( $app )
...
=head2 if_plugin( $component )

=head2 is_plugin( $component )
...
=head2 if_writable( $path, $blog )

=head2 is_writable( $path, $blog )
...
=head2 upload_callback( $app, $blog, $asset, $id )
...
=head2 register_template( $identifier, $path, \%param )

=head2 register_template( $identifier, \%path_info, \%param )
...
=item * $path - ( required )
    ...
    テンプレートオブジェクトのtextカラムに入る内容の元になるファイルのパスを指定します。
    パスは $params->{ component }->path の tmpl 下の相対パスか絶対パスで指定可能です。
    textカラム以外の場所にも登録したい場合は、

    { text => 'textカラム用のファイルパス', subject => 'subjectカラム用のファイルパス' }

    という形で、ハッシュリファレンスを渡すことにより、別のファイルを指定することが可能です。
    ハッシュリファレンスを渡す場合はtextキーは必須になります。

=item * $params - ( optional )

    デフォルト以外の動作を指定する場合のパラメータ
    下記以外のパラメータを指定した場合は、テンプレートの該当カラムに入れるものとして動作します。

=over 4

=item * $blog_id - ( optional; default 0 )

    テンプレートを検索・登録するブログのID

=item * $component - ( optional; default MT->app )

    テンプレートパス決定に使用するプラグイン

=item * $type - ( optional; default 'custom' )

    登録するタイプ

=item * $name - ( optional; default 'identifier' )

    テンプレート登録名

=item * $rebuild_me - ( optional; default 0 )

=back

=back

=head2 register_templates_to( $blog_id, $component, \%templates )
...
=head2 load_registered_template( $identifier, $path, \%param )
...
=head2 load_registered_template_for( $blog_id, $component, $identifier, \%templates )
...
=head2 str2array( $str, $separator, $remove_space )
...
=item * $separator - ( required; default ',' )

=item * $remove_space - ( optional )

    配列のアイテムからスペースを削除します。

=back

=head2 log2text( $msg, $out )

    テキストログをファイルに( 追記して )出力します。

=head2 get_config_inheritance( $plugin, $key, $blog )
...
=head2 plugin_template_path( $plugin, $dirname )

    プラグインディレクトリ直下の $dirname ( 省略の場合 'tmpl' ) ディレクトリへのフルパスを返します。

=head2 get_asset_from_text( $text, $blog )

    テキストからhttpではじまるURLへのリンクを抽出してテキストに含まれる MT::Asset オブジェクトの配列を返します。

=head2 convert2thumbnail( $blog, $text, $type, $embed [ , $link, $dimension, $convert_gif_png ] )
...
=item * $dimension - ( optional; default 'width' )

    width または height

=item * $convert_gif_png - ( optional )

    作成されたサムネイルのパスを convert_gif_png に渡します。

=back

=head2 create_thumbnail( $blog, $asset, %param )
